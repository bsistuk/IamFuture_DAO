...
class Election extends React.Component {
    ...
    async init (nextProps) {
        if (nextProps.account) {
            ...
            const vote = await daoContract.methods.votesHistory(this.props.account, (election.id / 1)).call();
            election.vote = vote;
    ...
    async vote(proposalID, trueOrFalse) {
        ...
        if (balance === 0) {
            alert ("you are not a member of Fair DAO")
            return
        }
                ...
                >
                    <Stack flexDirection="row" alignItems="flex-start" flex={3} gap={1}>
                        <IconButton size="small" 
                            ...
                            sx={{ bgcolor: this.props.theme.palette.mode === 'dark' ? '#323232' : '#EDEDED'
                        }}>
                            <ChevronLeftIcon fontSize="small" />
                        </IconButton>
                        <Box>
                            <Typography variant="subtitle1">{this.state.election.name}</Typography>
                            <Stack flexDirection="row" alignItems="center" gap={1} sx={{ pt: 1 }}>
                                ...
                                >{this.state.election.isVoteEnded ? 'Ended' : 'Active'}</Typography>} variant="outlined" size="small" />
                                <Chip label={<Typography variant="caption" 
                                    color="paper"
                                    sx={{ 
                                        color: 
                    ...
                    <Stack flex={1} gap={2.5}>
                        ...
                        <Box>
                            ...
                            >Vote :&nbsp;<strong>NO</strong></Button>
                        </Box>
                        {this.state.admin !== '' && (this.state.admin === this.props.account || this.state.owner === this.props.account) 
                        ?
                            <Box>
                                <Button 
                                    fullWidth
                        ...
                        <Stack sx={{ borderRadius: 2, border: `1px solid ${this.props.theme.palette.divider}`, width: '100%', height: 220, p: 2 }} justifyContent="center" alignItems="center">
                            <Box component="img" src={this.state.election.source ? this.state.election.source : "/images/election.png"} sx={{ maxWidth: '100%', maxHeight: '100%' }} />
                        </Stack>
                        <Box sx={{ p: 2, borderRadius: 2, border: `1px solid #CBCBCB` }}>
                            <Stack gap={2.5}>
                                <Typography variant="body2" sx={{ textTransform: 'uppercase' }}>Details</Typography>
                                <Stack gap={3}>
                                    ...
                                    <Stack flexDirection="row" alignItems="center" justifyContent="space-between">
                                        <Typography variant="body2" sx={{ fontWeight: 300 }}>Started</Typography>
                                        <Typography variant="body2" sx={{ fontWeight: 500 }}>{this.state.election.createdAt ? unixStamp(this.state.election.createdAt / 1) : ''}</Typography>
                                    </Stack>
                                    <Stack flexDirection="row" alignItems="center" justifyContent="space-between">
                                        <Typography variant="body2" sx={{ fontWeight: 300 }}>Ends</Typography>
                                        <Typography variant="body2" sx={{ fontWeight: 500 }}>Jul 13 00:05</Typography>
                                    </Stack>
                                    {/* <Stack flexDirection="row" alignItems="center" justifyContent="space-between">
                        ...
                        <Box sx={{ borderRadius: 2, border: `1px solid #CBCBCB` }}>
                            <Stack gap={2.5} sx={{ p: 2 }}>
                                <Stack flexDirection="row" alignItems="center" justifyContent="space-between">
                                    <Typography variant="body2" sx={{ textTransform: 'uppercase' }}>Current Result</Typography>
                                    <Typography variant="body2" sx={{ textTransform: 'uppercase' }}>See votes</Typography>
                                </Stack>
                                <Box>
                                    <Stack flexDirection="row" alignItems="center" justifyContent="space-between">
                                        <Typography variant="body2">Yes</Typography>
                                        <Typography variant="body2">{this.state.election.voteAmount ? ((this.state.election.votesForYes / 1)/(this.state.election.voteAmount)* 100).toFixed(): 0}%</Typography>
                                    </Stack>
                                    <LinearProgress color="success" variant="determinate" value={this.state.election.voteAmount ? ((this.state.election.votesForYes / 1)/(this.state.election.voteAmount)* 100).toFixed(): 0} />
                                    <Stack flexDirection="row" sx={{ pt: 1 }}>
                                        <Typography variant="caption">1,327,289</Typography>
                                        <Typography variant="caption">({this.state.election.votesForYes ? this.state.election.votesForYes / 1 : 0} votes)</Typography>
                                    </Stack>
                                </Box>
                                <Box>
                                    <Stack flexDirection="row" alignItems="center" justifyContent="space-between">
                                        <Typography variant="body2">No</Typography>
                                        <Typography variant="body2">{this.state.election.voteAmount ? ((this.state.election.votesForNo / 1)/(this.state.election.voteAmount)* 100).toFixed(): 0}%</Typography>
                                    </Stack>
                                    <LinearProgress color="error" variant="determinate" value={this.state.election.voteAmount ? ((this.state.election.votesForNo / 1)/(this.state.election.voteAmount)* 100).toFixed(): 0} />
                                    <Stack flexDirection="row" sx={{ pt: 1 }}>
                                        <Typography variant="caption">143,766</Typography>
                                        <Typography variant="caption">({this.state.election.votesForNo ? (this.state.election.votesForNo / 1): 0} votes)</Typography>
                                    </Stack>
                                </Box>
                                <Box sx={{ p: 2,
                                    pb: 0,
                                    ...
                                    <Carousel showArrows={false} showThumbs={false} showStatus={false} autoPlay={true}>
                                        <Stack sx={{ borderRadius: 2, px: 2, py: 1, bgcolor: this.props.theme.palette.divider }}>
                                            <Typography variant="caption" sx={{ fontWeight: 300 }}>Accpetance Threshold</Typography>
                                            <Typography variant="caption" sx={{ fontWeight: 500 }}>2,000,000 VP</Typography>
                                        </Stack>
                                        <Stack sx={{ borderRadius: 2, px: 2, py: 1, bgcolor: this.props.theme.palette.divider }}>
                                            <Typography variant="caption" sx={{ fontWeight: 300 }}>Needed for Acceptance</Typography>
                                            <Typography variant="caption" sx={{ fontWeight: 500 }}>671,711 VP</Typography>
                                        </Stack>
                                    </Carousel>
                                </Box>
                            </Stack>
                            ...
                            <Stack flexDirection="row" justifyContent="center" alignItems="center" sx={{ py: 3 }}>
                                <Button color="error" sx={{ textTransform: 'uppercase' }}>Sign in</Button>
                            </Stack>
                        </Box>
                    </Stack>
                </Stack>
...
const withHook = (Component) => {
    return (props) => {
        ...
        return (
            <Component theme={theme} position={position} account={account} navigate={navigate} params={params} {...props} />
        )
    }
}
